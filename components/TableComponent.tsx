        ...
        <div className="mx-auto mt-4">
            ...
            <div className="relative overflow-x-auto rounded-lg border">
                <table className="w-full text-sm text-left">
                    <thead>
                    <tr className="">
                        {!readonly && <th className="p-4 w-12 border-r">
                        ...
                        {fields.map((field) => (
                            ...
                            <th key={field} className="p-4 border-r">
                    ...
                    {data.map((record) => (
                        <tr key={record.id}>
                            {!readonly && <td className="p-4 border-t border-r">
                                <CustomTableBulkCheckbox
                            ...
                            {fields.map((field: any) => (
                                (!field.endsWith("_id") && !["password"].includes(field)) &&
                                <td key={field} className="border-t border-r py-2 px-4">
                                    {labels?.includes(field) && record[field] ?
                                        <StatusLabel status={record[field]}/> : record[field]}
                                </td>
                            ))}
                            {!readonly && <td className="border-t p-1">
                                <button
                                    className="text-yellow-600 dark:text-yellow-300 px-2 mr-4 py-0.5 rounded border"
                                    onClick={() => openCreateOrUpdateDialog(record)}
                                >
                                    Edit
                                </button>
                                <button
                                    className="dark:text-red-400 text-red-600 px-2 py-0.5 rounded border"
                                ...
                                {actions && actions.map(action => (
                                    <button
                                        key={action.key}
                                        className="px-2 ml-4 py-0.5 rounded border"
                ...
                <div>
                    {error &&
                        <div className="p-6 border-t  text-center text-red-500">{error}</div>
                    }
                    ...
                    {!error && !loading && data.length === 0 &&
                        <div className="border-t  text-center p-3 text-gray-500">There are no
                            records</div>
                    }
                </div>
                {loading &&
                    <div className="p-6 my-24 min-w-max absolute left-1/2 top-0"><Loader/></div>}
            </div>
                ...
                <Dialog as="div" className="relative z-10" onClose={closeDialogs}>
                    ...
                    <div className="fixed inset-0 overflow-y-auto">
                        <div className="flex items-center justify-center min-h-full p-4 text-center">
                            ...
                            >
                                <Dialog.Panel
                                    className="w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform shadow-xl bg-background rounded-2xl">
