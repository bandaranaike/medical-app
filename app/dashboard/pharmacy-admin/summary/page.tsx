...
const PharmacyAdminPortal: React.FC = () => {
    ...
    return (
        <div>
            ...
            <div className="relative overflow-x-auto rounded-lg border mt-8">
                <table className="w-full text-sm text-left">
                    ...
                    {drugStockSaleData && drugStockSaleData.length > 0 && drugStockSaleData.map((item) => (
                        <tr key={`${item.id}-${item.brand_name}`} className="border-t">
                            <td className="px-4 py-2 border-r">{item.drug_name}</td>
                            <td className="px-4 py-2 border-r">{item.brand_name}</td>
                            ...
                            <td className="px-4 py-2 border-r">{item.sale_quantity}</td>
                            <td className="px-4 py-2 border-r">{item.unit_price}</td>
                            <td className="px-4 py-2 border-r">{item.cost}</td>
                            <td className="px-4 py-2 border-r">{item.expire_date}</td>
                        </tr>
                    ))}
                    </tbody>
                </table>
